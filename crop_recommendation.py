...
excel_path = "D:/THARUN/PROJECTS/Capstone/Weather Data.xlsx"
weather_data = pd.read_excel(excel_path)

# Load the trained model and label encoder
model = joblib.load("D:/THARUN/PROJECTS/Capstone/model_rfc.pkl")
labelencoder_y = joblib.load("D:/THARUN/PROJECTS/Capstone/label_encoder.pkl")
...
st.subheader("Provide soil and environmental details to get the best crop recommendations!")
...
with col1:
    N = st.number_input("🧪 Nitrogen (N):", min_value=0, max_value=200, step=1, key="nitrogen")
with col2:
    P = st.number_input("🧪 Phosphorus (P):", min_value=0, max_value=200, step=1, key="phosphorus")
with col3:
    K = st.number_input("🧪 Potassium (K):", min_value=0, max_value=200, step=1, key="potassium")

# Row 2: Temperature, Humidity, pH level
with col1:
    temperature = st.number_input("🌡️ Temperature (°C):", value=avg_temp, max_value=50.0, step=0.1, key="temperature")
with col2:
    humidity = st.number_input("💧 Humidity (%):", value=avg_humidity, max_value=100.0, step=0.1, key="humidity")
with col3:
    ph = st.number_input("🧪 pH level:", min_value=0.0, max_value=14.0, step=0.1, key="ph")

# Row 3: Rainfall
with col1:
    rainfall = st.number_input("🌧️ Rainfall (mm):", value=avg_rainfall, max_value=500.0, step=1.0, key="rainfall")
...
if st.button("🔍 Predict Crop"):
    ...
    missing_fields = [field for field, value in fields.items() if value is None or value == 0]
